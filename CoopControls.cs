...
namespace IronNestVR
{
    ...
    internal static class CoopControls
    {
        ...
        {
            // PvP: control sync now runs for TEAMMATES too — a team SHARES one turret. We still broadcast
            ...
            // shot is adjudicated once by its shooter — sharing the gun is a later step.
        ...
        {
            // PvP: apply TEAMMATE control state too. Remote ownership/state is only ever set by packets
        ...
        // PvP TEAM ISOLATION: in a PvP lobby co-op replication is TEAM-SCOPED — a packet from a
        ...
        // host-stamped trailer on a relayed packet). Unused by the control handlers at the 2-player cap; a
        // per-peer build keys per-control ownership on it so a release from player B doesn't clear player C's lock.
        public static void OnPacket(byte type, ulong origin, Il2CppStructArray<byte> a, int len)
        {
            // PvP team gate: drop a non-teammate's co-op packet at LOCAL APPLY. The host already relayed
